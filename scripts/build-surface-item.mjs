...
//
// THE ENCODER HALF ONLY, since Modu-Portal landed. The portal owns publishing — its workshop form collects the tiling/offset/plinth fields and writes the item_surfaces row through publish_workshop_draft (migration 019) — but its surface form takes `accept=".ktx2"` and only VALIDATES what it is given, so nothing over there turns a raw PBR pack into those maps. This script is that step, and it stays the executable statement of what the app requires of them. Every encoding decision below is a contract the app depends on, so change them here and in the portal together.
//
...
// --- what the operator types into the portal's surface form -------------------
// Tiling is the map's PHYSICAL scale and CANNOT be derived from the pixels — it is how many times the texture repeats across the surface, and only a human knows how big a real patio tile is. These are starting points sized against the room's 4.5 m walls: tune them by eye in the form, and note that re-tuning is a field edit, never a re-upload.
...
// PRINTED, not written as a SQL upsert any more. item_surfaces is now reached exactly one way — the portal's workshop form, published through publish_workshop_draft — and a second writer that inserts straight into the table would be a way to create a row the draft flow never saw and cannot re-publish over. What survives is the part the form cannot work out for itself: has_normal/has_rough follow from which maps this run actually published, and the plinth tint is measured off the pixels. Offsets stay 0 and the trim fields stay empty — a cornice needs its own maps, which this script does not yet produce from a single-material source pack.
console.log(`\n-- ${itemDir}`);
console.log(`   1. upload the .ktx2 files above into the portal's surface form for '${itemId}'`);
console.log(`   2. tiling: scale ${scaleX} x ${scaleY}, offset 0 x 0`);
console.log(`   3. maps present: normal ${published.includes("normal")}, rough ${published.includes("rough")}`);
if (edge) console.log(`   4. plinth tint: rgb(${edge.join(", ")}) — the base colour's average, override by eye`);
console.log(`   ${edge ? 5 : 4}. still needed: a tile picture (square, transparent) for the shop and inventory tile\n`);
