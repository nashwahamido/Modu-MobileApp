...
import { CartIcon, CheckIcon, ChevronIcon, CloseIcon, CoinMedalIcon, FriendsIcon, InventoryIcon, LevelStarIcon, RotateLeftIcon, RotateRightIcon, SettingsIcon, ToolsIcon, TrashIcon } from './Icons';
...
export function RoomExperience() {
  ...
  const rotateRoom = (direction: -1 | 1) => setRoomRotation((value) => value + direction * Math.PI / 10);
  const zoomRoom = (direction: -1 | 1) => setRoomZoom((value) => Math.max(.72, Math.min(1.34, value + direction * .12)));

  return <View style={s.screen}>
    <View style={s.stage}><RoomScene rotationY={roomRotation} zoom={roomZoom} onRotationChange={setRoomRotation} /></View>
    <View style={s.stats}>
      <Pressable accessibilityLabel="Settings" style={s.settingsButton}><SettingsIcon size={40} color="#665f55"/></Pressable>
    ...
    <View style={s.roomControls}>
      <View style={s.controlGroup}>
        <Pressable accessibilityLabel="Rotate room left" style={s.controlButton} onPress={() => rotateRoom(-1)}><RotateLeftIcon size={23}/></Pressable>
        <Pressable accessibilityLabel="Rotate room right" style={s.controlButton} onPress={() => rotateRoom(1)}><RotateRightIcon size={23}/></Pressable>
      </View>
      <View style={s.controlGroup}>
        <Pressable accessibilityLabel="Zoom room in" style={s.controlButton} onPress={() => zoomRoom(-1)}><Text style={s.controlSymbol}>＋</Text></Pressable>
        <Pressable accessibilityLabel="Zoom room out" style={s.controlButton} onPress={() => zoomRoom(1)}><Text style={s.controlSymbol}>－</Text></Pressable>
      </View>
    </View>
...
const s=StyleSheet.create({
  ...
  roomControls:{position:'absolute',zIndex:13,left:24,bottom:18,flexDirection:'row',alignItems:'center',gap:10},controlGroup:{height:44,flexDirection:'row',alignItems:'center',gap:6,borderWidth:.65,borderColor:'#d8cdbb',borderRadius:14,backgroundColor:'rgba(251,248,243,.96)',paddingHorizontal:7,shadowColor:'#8d8190',shadowOpacity:.14,shadowRadius:5,shadowOffset:{width:-2,height:4}},controlButton:{width:34,height:34,borderRadius:17,alignItems:'center',justifyContent:'center',backgroundColor:'#F3ECE0'},controlSymbol:{color:'#231F20',fontSize:23,lineHeight:25,fontWeight:'800'},
