...
const roomVertexShader = `
  ...
  void main() {
    ...
    vLight = 0.48 + 0.52 * max(dot(transformedNormal, lightDirection), 0.0);
...
function roomFragmentShader(color: string) {
  ...
  return `
    ...
    varying float vLight;
    void main() {
      gl_FragColor = vec4(${red.toFixed(5)} * vLight, ${green.toFixed(5)} * vLight, ${blue.toFixed(5)} * vLight, 1.0);
    }
  `;
}
...
export function RoomScene() {
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onPanResponderMove: (_, gesture) => { setRotationY(rotationStart.current + gesture.dx * 0.009); },
  }), []);
  return (
    <View style={styles.container}>
      <Canvas style={styles.canvas} gl={{ alpha: true }} camera={{ position: [11, 8, -12], fov: 32, near: 0.1, far: 100 }}>
        ...
        <hemisphereLight args={['#fff8ec', '#67717a', 1.15]} />
        <directionalLight position={[-5, 10, -7]} intensity={1.65} />
