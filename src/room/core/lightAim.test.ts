...
// THE CROSS-REPO FIXTURE, copied verbatim from migration 014_light_aim.sql, with the workshop portal carrying the same table.
// The only thing standing between the two repos and a silent convention mismatch: flip a sign either side and that side's own test fails, rather than every lamp shipping aimed backwards.
//
...
// Coverage, the question the representation had to answer before it was chosen: two angles must reach every direction on the sphere, or some lamp is unaimable.
...
// A point light stores NULL for both angles, and the catalog is a network fetch: a lamp aimed the wrong way is a bug someone reports, a crash on entering the room is not.
