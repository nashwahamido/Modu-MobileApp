...
// How much "in front" a wall must get before it is fully gone, as a dot-product width — about 7 degrees of orbit past the moment it starts to block.
...
// Wall-mounted furniture (windows, photo frames) fades on its wall's OWN alpha — wallAlpha(wall, theta), no separate curve and no threshold. There is nothing to export for it, and that is the point: the moment an item has a schedule of its own, it disagrees with the wall somewhere in the band, and a window that empties out of a wall still standing at half alpha is exactly the glitch this replaced.
// It used to POP, at the fade midpoint with hysteresis, because furniture GLBs ship OPAQUE frame materials and alpha written to an opaque material is discarded — only the glass would have dimmed while the frame hung solid in mid-air. That is an ASSET fact, not a renderer one, and it is now fixed where it lives: scripts/set-item-blend-modes.mjs re-declares a wall item's materials as BLEND before upload, which is what makes the frame fadeable at all. A wall item whose GLB has not been through that script holds full opacity through its wall's fade and then vanishes with it — degraded, not broken, and the way to recognise it is that it looks like the old pop.
// The renderer drops a faded-out item from the scene at WALL_ALPHA_EPSILON rather than leaving it in at alpha 0. No hysteresis is needed on that boundary any more, and its absence is the proof the fade works: on either side of the threshold the item is equally invisible, so a camera parked exactly on it can flicker between the two states all it likes and there is nothing to see.
