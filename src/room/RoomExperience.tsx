...
export function RoomExperience() {
  ...
  const collisionFootprint = roomFurniture?.footprint ?? DEFAULT_FURNITURE_FOOTPRINT;
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onPanResponderMove: (_, g) => {
      ...
      const candidateScale = getPlacementPerspectiveScale(candidate.y, height);
      const warning = getPlacementIssue(candidate, width, height, candidateScale, collisionFootprint);
  ...
  const roomPivot = { x: width * .5, y: height * .57 };
  ...
  const projectedOffsetX = Animated.add(
    ...
    Animated.multiply(roomOffsetY, rotationSin * .52),
  );
  const projectedOffsetY = Animated.add(
    roomOffsetY,
    Animated.multiply(roomOffsetX, rotationSin * .18),
  );
  ...
  return <View style={s.screen}>
    <View style={s.stage}>{heavySceneActive ? null : <RoomScene
      rotationY={roomRotation}
      zoom={roomZoom}
      onRotationChange={handleRoomRotationChange}
      furniture={roomFurniture && (placing || placed) ? {
        itemId: roomFurniture.id,
        position: normalizePlacementPoint(placementPoint, width, height),
        perspectiveScale: getPlacementPerspectiveScale(placementPoint.y, height),
        renderStyle,
      } : null}
    />}</View>

    {placing && roomFurniture ? <View
    ...
    {(placing || placed) && !roomFurniture ? <Animated.View
      {...panResponder.panHandlers}
      style={[
        s.furnitureWrap,
        { transform:[{translateX:furnitureTranslateX},{translateY:furnitureTranslateY},{scale:furnitureScale}] },
        placing && s.furnitureActive,
        placementWarning && s.furnitureBlocked,
      ]}
    ><Pressable
      accessibilityLabel={placing ? `Move ${itemName ?? 'furniture'}` : 'Edit furniture'}
      style={s.furniturePressTarget}
      onPress={editPlacement}
    ><Cabinet/></Pressable></Animated.View> : null}

    {firstPlacementGuide ? <View pointerEvents="none" style={s.firstPlacementGuide}>
      <Image source={mascot} style={s.guideMascot} resizeMode="contain" />
      <View style={s.guideCopy}>
        <Text style={s.guideTitle}>Place your new {itemName ?? 'furniture'}</Text>
        <Text style={s.guideBody}>Drag it to a spot you like, then tap the checkmark.</Text>
      </View>
    </View> : null}
    ...
    {placing ? <View style={[s.placeBar, placementWarning && s.placeBarWarning]}><Text style={[s.placeHint, placementWarning && s.placeWarningText]}>{placementWarning ?? 'Drag to position'}</Text><Pressable accessibilityLabel="Delete furniture" style={s.deleteButton} onPress={removePlacement}><TrashIcon size={23} color={t.danger}/></Pressable><Pressable accessibilityLabel="Confirm furniture position" style={s.confirm} onPress={handleConfirmPlacement}><CheckIcon size={27} color={t.onSuccess}/></Pressable></View>:null}
...
const makeStyles = (t: Theme) => StyleSheet.create({
  ...
  furnitureWrap:{position:'absolute',zIndex:10,left:-55,top:-45,width:110,height:90},furniturePressTarget:{flex:1,alignItems:'center',justifyContent:'center'},furnitureActive:{borderWidth:2,borderStyle:'dashed',borderColor:t.accent,borderRadius:12},furnitureBlocked:{borderColor:t.danger,backgroundColor:'rgba(187, 67, 67, .12)'},firstPlacementGuide:{position:'absolute',zIndex:11,left:24,top:76,width:300,minHeight:66,flexDirection:'row',alignItems:'center',gap:10,borderWidth:2,borderColor:t.success,borderRadius:12,backgroundColor:t.surface,paddingVertical:8,paddingHorizontal:10,shadowColor:t.success,shadowOpacity:.26,shadowRadius:8,shadowOffset:{width:0,height:3}},guideMascot:{width:48,height:48,borderRadius:10},guideCopy:{flex:1,gap:2},guideTitle:{color:t.text,fontSize:14,fontWeight:'800'},guideBody:{color:t.textDim,fontSize:11,lineHeight:14,fontWeight:'600'},placeBar:{position:'absolute',zIndex:12,bottom:78,alignSelf:'center',maxWidth:'58%',minHeight:48,flexDirection:'row',alignItems:'center',gap:10,borderRadius:22,backgroundColor:t.surface,paddingLeft:18,paddingRight:6,paddingVertical:6,shadowColor:'#000',shadowOpacity:.18,shadowRadius:8},placeBarWarning:{borderWidth:2,borderColor:t.danger},placeHint:{flexShrink:1,color:t.textDim,fontWeight:'600'},placeWarningText:{color:t.danger,fontSize:11},deleteButton:{width:36,height:36,borderRadius:18,backgroundColor:t.surfaceRaised,alignItems:'center',justifyContent:'center'},confirm:{width:36,height:36,borderRadius:18,backgroundColor:t.success,alignItems:'center',justifyContent:'center'},
