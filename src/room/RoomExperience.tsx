...
export function RoomExperience() {
  ...
  return <View style={s.screen}>
    ...
    {placing ? <View style={s.placeBar}><Text style={s.placeHint}>Drag to position</Text><Pressable accessibilityLabel="Delete furniture" style={s.deleteButton} onPress={deleteFurniture}><TrashIcon size={23} color={t.danger}/></Pressable><Pressable accessibilityLabel="Confirm furniture position" style={s.confirm} onPress={confirmPlacement}><CheckIcon size={27} color={t.onSuccess}/></Pressable></View>:null}

    {unavailableFeature ? <View style={s.overlay}><View style={s.comingSoonCard}><Text style={s.comingSoonTitle}>{unavailableFeature}</Text><Text style={s.comingSoonBody}>This feature is coming soon.</Text><Button label="Got it" variant="primary" style={s.comingSoonButton} onPress={()=>setUnavailableFeature(null)}/></View></View>:null}

    {inventoryOpen ? <View style={s.overlay}><View style={s.inventory}><View style={s.inventoryHeader}><View><Text style={s.inventoryTitle}>Inventory</Text><Text style={s.inventorySubtitle}>Furniture</Text></View><Pressable onPress={()=>setInventoryOpen(false)}><CloseIcon size={30} color={t.text}/></Pressable></View><View style={s.grid}>{products.map((name,index)=><Pressable key={name} style={s.card} onPress={index===0?chooseCabinet:undefined}><View style={s.preview}>{index===0?<Cabinet/>:<View style={s.emptyPreview}/>}</View><Text style={s.cardName}>{name}</Text></Pressable>)}</View></View></View>:null}
  </View>;
}
...
const makeStyles = (t: Theme) => StyleSheet.create({
  ...
  rail:{position:'absolute',zIndex:14,right:18,top:58,width:78,height:238,paddingVertical:15,borderWidth:1.2,borderColor:t.border,borderRadius:18,backgroundColor:t.surface,alignItems:'center',justifyContent:'space-between',shadowColor:'#8d8190',shadowOpacity:.17,shadowRadius:7,shadowOffset:{width:-3,height:6}},railClosed:{top:22,width:78,height:52,paddingVertical:0,borderWidth:0,backgroundColor:'transparent',shadowOpacity:0},chevron:{position:'absolute',top:-34,width:48,height:34,alignItems:'center',justifyContent:'center'},chevronClosed:{top:0,width:58,height:30},collapsedPill:{position:'absolute',top:39,width:58,height:9,borderRadius:6,backgroundColor:t.surface,borderWidth:.65,borderColor:t.border,shadowColor:'#8d8190',shadowOpacity:.18,shadowRadius:4,shadowOffset:{width:-2,height:3}},railButton:{width:74,minHeight:58,alignItems:'center',justifyContent:'center'},railLabel:{fontSize:10.5,lineHeight:13,color:t.text,marginTop:1,fontWeight:'400',textAlign:'center'},workbench:{position:'absolute',zIndex:13,right:20,bottom:18,width:162,height:44,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:13,borderWidth:.65,borderColor:t.border,borderRadius:13,backgroundColor:t.surface,shadowColor:'#8d8190',shadowOpacity:.16,shadowRadius:5.5,shadowOffset:{width:-3,height:5}},workbenchText:{fontSize:15.5,fontWeight:'600',color:t.text},
  furnitureWrap:{position:'absolute',zIndex:10,left:-55,top:-45,width:110,height:90},furnitureActive:{borderWidth:2,borderColor:t.accent,borderRadius:8},cabinet:{width:110,height:78,backgroundColor:'#b8783f',borderWidth:2,borderColor:'#754621'},topWood:{height:10,backgroundColor:'#d79a58'},drawers:{height:56,flexDirection:'row',flexWrap:'wrap'},drawer:{width:'33.33%',height:'50%',borderWidth:.7,borderColor:'#754621',alignItems:'center',justifyContent:'center'},knob:{width:4,height:4,borderRadius:2,backgroundColor:'#58331b'},legs:{height:12,flexDirection:'row',justifyContent:'space-between',paddingHorizontal:8},leg:{width:6,height:12,backgroundColor:'#754621'},placeBar:{position:'absolute',zIndex:12,bottom:78,alignSelf:'center',flexDirection:'row',alignItems:'center',gap:10,borderRadius:22,backgroundColor:t.surface,paddingLeft:18,paddingRight:6,paddingVertical:6,shadowColor:'#000',shadowOpacity:.18,shadowRadius:8},placeHint:{color:t.textDim,fontWeight:'600'},deleteButton:{width:36,height:36,borderRadius:18,backgroundColor:t.surfaceRaised,alignItems:'center',justifyContent:'center'},confirm:{width:36,height:36,borderRadius:18,backgroundColor:t.success,alignItems:'center',justifyContent:'center'},
  overlay:{...StyleSheet.absoluteFillObject,zIndex:20,backgroundColor:t.scrim,alignItems:'center',justifyContent:'center'},comingSoonCard:{width:300,borderRadius:22,backgroundColor:t.surface,padding:24,alignItems:'center',shadowColor:'#000',shadowOpacity:.18,shadowRadius:12},comingSoonTitle:{fontSize:22,fontWeight:'900',color:t.text},comingSoonBody:{marginTop:8,fontSize:14,fontWeight:'600',color:t.textDim},comingSoonButton:{marginTop:18,minWidth:120},inventory:{width:'82%',height:'84%',borderRadius:24,backgroundColor:t.surface,padding:22},inventoryHeader:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:12},inventoryTitle:{fontSize:24,fontWeight:'700',color:t.text},inventorySubtitle:{fontSize:13,color:t.textDim,marginTop:2},grid:{flex:1,flexDirection:'row',flexWrap:'wrap'},card:{width:'25%',height:'50%',padding:8,alignItems:'center'},preview:{width:'86%',flex:1,maxHeight:90,borderWidth:1.5,borderColor:t.border,borderRadius:10,backgroundColor:t.surfaceInset,alignItems:'center',justifyContent:'center',overflow:'hidden'},emptyPreview:{width:45,height:45,borderWidth:1.5,borderColor:t.borderStrong,transform:[{rotate:'45deg'}]},cardName:{fontSize:11,color:t.textDim,marginTop:5,textAlign:'center'},
});
