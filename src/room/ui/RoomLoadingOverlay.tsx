// The wait in front of a room — the player's own in the hub, a friend's on the visit screen. Both screens mount their RoomScene UNDER this and keep it up until the whole room has landed, so a room is never watched assembling itself: no bare shell, no furniture popping in one piece at a time.
// It owns no look of its own. LoadingScreen is the app's single loading look (mascot/initial ring, name line, creep/jump bar) and this is the thin piece that maps a room's two waits onto its Milestone — which is the same job LoadingOverlay does for the assembly loader.
import { useEffect, useState } from "react";

import { LoadingScreen, type LoadingAvatar } from "@/src/game/ui/LoadingScreen";
