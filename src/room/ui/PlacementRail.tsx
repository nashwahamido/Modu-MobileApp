...
function blockedHint(reason: string | null): string {
  ...
  switch (reason) {
    case "occupied":
      return "That spot is occupied. Try another place.";
    case "out-of-bounds":
      return "Keep the furniture on the room floor.";
    case "surface-not-allowed":
      return "This piece cannot go there.";
    default:
      return "This item cannot be placed yet.";
  }
}
  ...
  return (
    ...
    <View
      ...
      style={[
        s.rail,
        {
          ...
          top:
            12 +
            Math.max(safe.raw.top, SCREEN_VERTICAL_MARGIN) +
            TOP_STATS_CLEARANCE,
          bottom:
            22 +
    ...
    >
      ...
      <View style={[s.bar, blocked && s.barBlocked]}>
        <Text style={[s.hint, blocked && s.hintBlocked]}>
        ...
        {ghostLightOn !== null ? (
          ...
          >
            <BulbIcon
              size={20}
              on={ghostLightOn}
              color={ghostLightOn ? "#8a6b1f" : "#807277"}
            />
          </Pressable>
        ) : null}
