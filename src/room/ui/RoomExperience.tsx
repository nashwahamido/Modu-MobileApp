...
export function RoomExperience() {
  ...
  const [lightOverride, setLightOverride] =
    useState<CeilingLightOverride>(null);
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(ROOM_WELCOME_GUIDE_KEY)
      ...
      .catch((err) =>
        console.warn("[room] welcome guide state load failed", err),
      );
    return () => {
      active = false;
    };
  ...
  const applyRoomControls = (nextRotation: number, nextZoom: number) => {
    const clampedZoom = Math.max(
      ORBIT.zoom.min,
      Math.min(ORBIT.zoom.max, nextZoom),
    );
  ...
  return (
    <View style={s.screen}>
      ...
      <SceneBackdrop
        {...roomBackdropView(roomBackdrop, darkTheme)}
        style={s.stage}
      >
        {sceneMounted ? (
          <RoomScene
      ...
      <RoomFirstPlacementGuide
        requestedItemId={
          firstPlacement === "lack-table" ? firstPlacement : null
        }
      ...
      {inventoryOpen ? (
        <InventoryOverlay onClose={() => setInventoryOpen(false)} />
      ) : null}

      {visitPickerOpen ? (
        <FriendPickerOverlay onClose={() => setVisitPickerOpen(false)} />
      ) : null}
      ...
      {showRoomEditGuide ? (
        <OverlaySheet size="dialog" onClose={dismissRoomEditGuide}>
          ...
          <Text style={s.roomGuideBody}>
            Press and hold any furniture to move, rotate, recolour, or remove
            it.
          </Text>
          <Button
            label="Got it"
