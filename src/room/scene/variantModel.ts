...
//
// The probe doubles as the CACHE-BUSTER. Storage serves models with cache-control: max-age=3600, and
// the device's HTTP cache honours it across app restarts — a re-uploaded model was invisible for up to
// an hour. The HEAD revalidates against origin (no-cache) and the file's ETag becomes a ?v= query on
// the URL the scene actually loads: a re-upload changes the ETag, which changes the URL, which is a
// cache miss by construction. Unchanged files keep their URL and their cached bytes.
...
// url -> the versioned URL to load (ETag-stamped), or false when the file is not in storage.
// Session-lifetime, like any other bundled-vs-remote decision.
...
function probe(url: string): Promise<string | false> {
  ...
  const p = fetch(url, { method: "HEAD", headers: { "Cache-Control": "no-cache" } })
    .then(
      // A real server answer (200 or 404) is stable for the session — cache it either way. A 200
      // with no readable ETag still works, just without busting (the bare URL keeps old cache
      // semantics rather than losing the model).
      (res): string | false => {
        if (!res.ok) {
