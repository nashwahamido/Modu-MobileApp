...
function paint(
  ...
  for (const name of names) {
    ...
    if (maps.normal) instance.setTextureParameter(renderableManager, MAP_PARAMS.normal, maps.normal);
    if (maps.rough) instance.setTextureParameter(renderableManager, MAP_PARAMS.rough, maps.rough);
...
export function applySurfaceItem({ slot, instances, renderableManager, maps, spec }: ApplySurfaceItemArgs): void {
  if (slot === "wall") {
    paint(instances, renderableManager, SHELL_GROUPS.walls, maps, spec.tiling);

    // The cornice sits at the wall/ceiling junction and follows the WALLPAPER, not the floor — it keeps its authored look unless the item shipped maps for it, which most wall items will not.
    if (maps.trim) paint(instances, renderableManager, SHELL_GROUPS.cornice, maps.trim, spec.trimTiling ?? spec.tiling);
    return;
  }

  paint(instances, renderableManager, SHELL_GROUPS.slab, maps, spec.tiling);
