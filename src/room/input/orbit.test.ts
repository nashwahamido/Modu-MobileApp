...
test("theta passes through untouched — the room turns all the way round", () => {
  // The yaw clamp existed because a two-wall shell showed its open back past ±45°. With four walls and camera-facing culling every azimuth reads as a room.
...
test("close-up zoom is allowed but never enters the near plane", () => {
  // 300% puts the eye at radius ~3.3 in a 2-unit scene: close, but still outside the room's bounding sphere plus the near plane.
...
test("rotationY keeps its old visual direction on the camera", () => {
  // Rotating the room +y swung the scene one way, and orbiting the camera the OPPOSITE way shows the same view — which is why theta runs against rotationY.
...
test("reset returns all three axes to the pose the room opens in", () => {
  ...
  // And exactly the neutral HUD state, since the reset reports back through the same (rotationY, zoom) pair the buttons speak.
...
test("reset takes the SHORT way home after the room has been spun round", () => {
  // theta is unbounded, so a player who has turned the room three times sits at restTheta - 6π. Resetting to the literal value spins the diorama three turns backwards, since the smoothed value chases raw in VALUE, not angle.
...
test("smoothing is frame-rate independent and matches the reference at 60fps", () => {
  ...
  // Degenerate frames never overshoot or go backwards: a huge dt saturates to exactly 1.0, which is convergence.
...
test("the eye orbits the target at the requested radius", () => {
  ...
  // At rest theta 3π/4 the camera sits in the +x/-z quadrant, outside the diorama's open corner, looking in.
  assert.ok(eye.x > 0);
  assert.ok(eye.z < 0);
...
test("the solved home radius keeps the telephoto framing", () => {
  // Solved for the 68mm lens across the whole orbit arc: much closer re-crops the room, much farther shrinks it needlessly.
