...
test("theta passes through untouched — the room turns all the way round", () => {
  // The yaw clamp existed because the shell had two walls and showed its open back past ±45°. With four walls and camera-facing culling every azimuth reads as a room, so nothing bounds theta.
...
test("close-up zoom is allowed but never enters the near plane", () => {
  // The user zooms in to inspect one piece: 300% puts the eye at radius ~3.3 in a 2-unit-cube scene — close, but still outside the room's bounding sphere plus the 0.1 near plane.
...
test("rotationY keeps its old visual direction on the camera", () => {
  // Rotating the room +y used to swing the scene one way; orbiting the camera the OPPOSITE way shows the same view, which is why theta runs against rotationY.
...
test("reset returns all three axes to the pose the room opens in", () => {
  ...
  // And it is exactly the neutral HUD state, since the reset reports itself back through the same (rotationY, zoom) pair the buttons speak.
...
test("reset takes the SHORT way home after the room has been spun round", () => {
  // theta is deliberately unbounded (see clampOrbit), so a player who has turned the room three times sits at restTheta - 6π. Resetting to the literal restTheta would spin the whole diorama three turns backwards, because the smoothed value chases raw in VALUE, not in angle.
...
test("smoothing is frame-rate independent and matches the reference at 60fps", () => {
  ...
  // Degenerate frames never overshoot or go backwards. (A huge dt saturates to 1.0 exactly in float64 — that is convergence, not overshoot.)
...
test("the eye orbits the target at the requested radius", () => {
  ...
  // At rest theta 3π/4 the camera sits in the +x / -z quadrant — outside the diorama's open corner (walls are at x-min and z-max), looking in.
  assert.ok(eye.x > 0);
  assert.ok(eye.z < 0);
...
test("the solved home radius keeps the telephoto framing", () => {
  // Regression guard: solved for the 68mm lens across the whole orbit arc (solve-orbit.mjs). Anything much closer re-crops the room; anything much farther shrinks it needlessly.
