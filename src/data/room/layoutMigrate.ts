...
// any unrecognised shape, version or legacy bare array becomes an EMPTY room rather than a mis-placed one
export function migrateRoomPlacements(envelope: unknown): PlacedFurniture[] {
  ...
  // canPlace dereferences placement.cell.x and surface.kind unconditionally, so one corrupt row must drop THAT row alone
...
// NOT VERSIONED, which is what made the field free to add: an older build reads only env.placements and renders the authored look
// validation against the CATALOGUE happens at the call site — this module only guarantees the SHAPE
...
export function readRoomFinishes(envelope: unknown): RoomFinishes {
  ...
  // per-slot, so one corrupt slot never costs the other — an empty string is dropped as hard as a number, being no id at all
