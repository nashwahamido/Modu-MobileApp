...
// which collapses a whole floor onto one texel, so a string is accepted only when it parses finite
...
// undefined is NOT an error path — it reads as an item with no surface data, and the shell renders as authored
// nothing throws here, because one malformed row must never fail the whole catalogue fetch
// each conditional group is re-checked as a unit and dropped whole, so a bad plinth colour costs the floor nothing
export function parseSurfaceSpec(raw: unknown): SurfaceItemSpec | undefined {
  ...
  // base colour is always present, so the booleans only ADD — an unknown map name is unrepresentable
  ...
  // all-or-nothing — without its own tiling the trim lands at the wall's scale, the look it exists to avoid
  if (r.has_trim === true) {
