...
//
// Layout — folder = id; NO light/dark theme (one image per variation, any UI mode). The subtree segment
// is the source tag verbatim, and thumbs are JPEG — both VERIFIED against the bucket, which is the
// authority here (an earlier guess of `buy/` + `.png` matched nothing):
//   room/<built|bought>/<id>/<variation|'default'>.glb   room placement model
//   room/<built|bought>/<id>/<variation|'default'>.jpg   its thumbnail (tile picture + picker swatch)
//   room/<built|bought>/<id>/tile.jpg                    item tile — surfaces; model-items reuse the default variation image
...
// Which item table an item comes from — and, verbatim, the room/<built|bought>/ subtree it lives in.
// Matches placeable_items.source.
export type ItemSource = "built" | "bought";

// One extension for every catalog image, in ONE place: the uploads are JPEG (photographic renders, and
// far smaller than PNG at this size). Change it here if that ever changes.
const IMAGE_EXT = ".jpg";
...
// variation) falls back to a dedicated tile image in its folder.
