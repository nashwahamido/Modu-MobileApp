...
//
// Layout — folder = id; NO light/dark theme (one image per variation, any UI mode). The subtree segment is the source tag verbatim, and thumbs are square transparent PNGs — the shop and inventory grids render them with resizeMode="contain" against a transparent surface (src/components/CatalogThumb.tsx), so a baked-in opaque background would fight the surrounding UI and an aspect-preserving rectangle would break the grid's per-item layout, which is why this is PNG and not the JPEG the bucket originally shipped: room/<built|bought|workshop>/<id>/<variation|'default'>.glb   room placement model room/<built|bought|workshop>/<id>/<variation|'default'>.png   its thumbnail (tile picture + picker swatch) room/<built|bought|workshop>/<id>/tile.png                    item tile — surfaces; model-items reuse the default variation image assembly/<id>/...                                    assembly build model (built items; separate tree). "workshop" is a testing-status draft, dev builds only — see the ItemSource comment below.
...
// One extension for every catalog image, in ONE place: PNG rather than JPEG so the render can carry real alpha — the shop/inventory grids composite thumbnails onto a transparent surface, and a JPEG can never supply that channel no matter how it is encoded. A surface tile (a full-bleed texture swatch, where alpha does nothing) pays for this in file size same as everything else; see the portal's src/tiles/tile.ts for the capture/encode side of this rule. Change it here if that ever changes.
const IMAGE_EXT = ".png";
...
//
// Every model-item takes thumbPath at its DEFAULT variation instead — including one-look items like the windows, whose variation is legitimately null and whose picture is default.png. A null variation therefore does NOT mean "surface", and a caller must decide between the two on the item's category rather than on the nullness of its variation.
