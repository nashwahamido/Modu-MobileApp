...
import { MALLET_TAPS, TIGHTEN_TOTAL_DEG, useGameStore } from '../game/store';
...
export function TapControl({ action, sinkDriver }: Props) {
  ...
  const tap = Gesture.Tap()
    ...
    .onEnd(() => {
      ...
      store.addTightenDeg(action.id, DEG_PER_TAP);
  ...
  return (
    <TutorialTarget id="tool" style={styles.wrap} pointerEvents="box-none">
      <GestureDetector gesture={tap}>
        <Animated.View style={[styles.target, { transform: [{ scale: squash }] }]} collapsable={false}>
