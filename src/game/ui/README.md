...
| `settings/` | `SettingsControls` (the controls themselves) and `GameSettings` (the in-build gear panel that hosts them). The settings ROUTE hosts the same controls, which is why they are a separate file. | On demand. |
