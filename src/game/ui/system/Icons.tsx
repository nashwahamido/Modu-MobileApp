...
//
// Two reasons this beats an image asset here: the colour follows the theme (a PNG has to be recoloured or tinted, and tinting is the thing that silently fails), and it stays crisp at any size on any density without exporting @2x/@3x variants.
...
export function RecenterIcon({ size = 20, color }: IconProps) {
  ...
  // The chevron spread is deliberately narrow (±2.9, not ±3.4): four wide arrowheads meeting at a point read as a snowflake rather than as arrows converging.
    ...
    <Svg width={size} height={size} viewBox="0 0 24 24">
      {[0, 90, 180, 270].map((deg) => (
        <Path
          ...
          stroke={color}
          // 1.6, not 2.1: at 20px the heavier stroke closed the gaps between the four arrows and the whole glyph filled in.
          strokeWidth={1.6}
          strokeLinecap="round"
...
export function StarBadge({ size = 40, color }: IconProps) {
  const outer = 11.5;
  // Fuller arms than a geometric star: the wireframe's badge is a chunky sticker, and a deep inner radius reads as spiky next to the round nodes.
  const inner = 5.4;
  const pts: string[] = [];
  for (let i = 0; i < 10; i++) {
