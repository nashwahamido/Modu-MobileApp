// The shared overlay primitive: a full-screen scrim with a centered card that slides up + fades
// in on mount. One source of truth for how modal overlays are framed — scrim, card radius,
// elevation, and the optional title/subtitle/close header — so every overlay reads as one system
// instead of each surface re-styling its own scrim and card. Render your content as children.
...
export function OverlaySheet({
  ...
  // Slide + fade on mount. The sheet is conditionally rendered by its caller, so mounting == opening.
  // "bottom" travels a full screen height (starts fully off-screen below); "center" is a small nudge.
  // Decelerate easing (fast start, gentle settle) so it reads like the OS modal, not a stiff linear glide.
  const anim = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    Animated.timing(anim, {
      toValue: 1,
      duration: anchor === "bottom" ? 360 : 260,
      easing: Easing.out(Easing.cubic),
      useNativeDriver: true,
    }).start();
  }, [anim, anchor]);
  const translateY = anim.interpolate({ inputRange: [0, 1], outputRange: [anchor === "bottom" ? height : 48, 0] });
