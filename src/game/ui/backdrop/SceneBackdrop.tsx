// The one backdrop renderer for every scene screen (play, tutorial, room) — an ImageBackground as the screen root, so the artwork always frames identically. A separate <Image style={absoluteFill}> scaled the same file differently (that was the tutorial's zoomed-in backdrop), so all screens must go through this component. The source is resolved by the caller's own backdrop table (game/ui/backdrops for the build screens, room/ui/roomBackdrops for the room) — this component only renders it, so the two sets can diverge without a shared id type. "clear": the resolver returns undefined — no image, the caller's root background shows through.
