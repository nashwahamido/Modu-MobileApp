...
export function BuildComplete() {
  ...
  return (
    <View
      style={[
        styles.scrim,
        // The card can fill the screen on a short landscape window, so the scrim carries the safe
        // margins rather than the card — nothing inside it can then reach a rounded corner or a notch.
        {
...
const makeStyles = (t: Theme) =>
  StyleSheet.create({
    ...
    // The ribbon is a SIBLING of the card, pulled down over it by a negative margin, rather than an
    // absolutely-positioned child: Android clips children that overflow their parent's bounds, and
    // half this banner sits above the card's top edge.
    ...
    // maxWidth, not just flex: the panels hold two short lines of text and a couple of icons, and
    // letting them grow with the card only pushes their own content further apart.
