...
export const pressPadStyles = StyleSheet.create({
  ...
  // fontFamily added deliberately: four of the five copies set only fontWeight, which React Native renders in the SYSTEM font rather than Lexend (see convention 5 in ui/theme). TapControl's copy already carried it and is the one that was right. The COLOUR is still disputed and is NOT resolved here — see hintInk below.
  hint: { fontFamily: FONT, fontSize: 12, color: "#FBF8F3", fontWeight: "700" },
  // TapControl's caption is dark where its four siblings' are cream, at the same screen position over the same scene, so one of the two is hard to read on any given backdrop. Preserved rather than unified, because picking one changes how a shipped control looks and that is a design call.
  hintInk: { color: "#6b6257", fontWeight: "600" },
});
