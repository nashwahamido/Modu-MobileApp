...
export function useAssemblyDrivers() {
  ...
  // The joystick deflection (-1..1 per axis), written by the stick gesture on the UI thread and integrated into the camera on the RENDER thread (scene/OrbitDrive). This is what keeps orbiting alive while a runOnJS part-drag saturates the JS thread — the old JS setInterval starved and the camera froze until the finger lifted.
