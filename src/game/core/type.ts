...
/** What a piece of hardware is FOR — the fact a human states about a fastener, and since 2026-09-01 the fact the runtime reads. It replaced `FastenerKind` ("threaded" | "pin" | "cam" | "secured"), which answered a different question (how the thing is DRIVEN) and answered it in four names that had to be decoded back into role + preload at every call site: `!== "secured"` meant "is a connector", `"threaded" || "cam"` meant "completes on tighten", `"threaded"` vs `"pin"` meant "the counterpart screws on" vs "presses on". Each reader re-derived a fact the author had already stated. */
export type FastenerRole = "connector" | "securer" | "cap" | "extra";

/** A connector's two ordering facts, and the reason connectors need a record where the other three roles do not. `completesOn` gates the preload lock (a pin frees its cluster when pressed in, a bolt only when driven home); `counterpartMountsBy` is what the LATER endpoint does on arrival — drop straight on, or dial on. Only ever present on role "connector". */
...
export interface FastenerFields {
  /** GENERATED by lowerFasteners from this part's group def, for EVERY fastener instance — not only where it disagrees with the name. The group-name prefix table (model/fastener-roles.json) survives as the extractor's PREFILL for a GLB nobody has authored yet; once a furniture states its FASTENERS, no runtime decision is left resting on a name. */
  fastenerRole?: FastenerRole;
  /** Present exactly when `fastenerRole` is "connector"; generated alongside it. */
