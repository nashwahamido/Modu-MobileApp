// Guide's stage offering must follow legally AVAILABLE work, not merely incomplete work
...
function bottomFirstTrap(): Set<ActionId> {
  ...
  for (let i = 0; i < 60; i++) {
    const avail = availableInMode(f, done, "free", CABINET).filter(
      (a) =>
        actionCluster(f, a) === CABINET &&
        a.actionId !== "place_backPanel" &&
        a.actionId !== "place_topPanel",
    );
    if (!avail.length) break;
    doAction(done, avail[0].actionId);
  }
  assert.ok(
    done.has("place_sidePanelL" as ActionId) &&
      done.has("place_sidePanelR" as ActionId),
    "trap fixture must reach both sides placed",
  );
  assert.ok(
    !done.has("place_backPanel" as ActionId) &&
      !done.has("place_topPanel" as ActionId),
    "trap fixture must leave back + top pending",
  );
  return done;
}
...
test("bottom-first EKET in guide offers the back panel instead of deadlocking", () => {
  ...
  assert.ok(
    guide.length > 0,
    "guide offering must never be empty while legal cabinet work exists",
  );
  assert.ok(
    guide.some((a) => a.actionId === "place_backPanel"),
    "the gate-mandated stage-2 back panel is the way forward",
  );
});
...
test("the TRAY follows the offering: the trapped state shows an enabled Back panel card", () => {
  ...
  assert.ok(
    back,
    `expected a backPanel card, tray has: ${scene.trayItems.map((t) => t.group).join(", ") || "(none)"}`,
  );
...
test("the stage floor never skips ahead of available same-stage work", () => {
  ...
  assert.ok(
    guide.every((a) => a.stage === 1),
    `expected only stage-1 offerings, got ${guide.map((a) => `${a.actionId}(s${a.stage})`).join(", ")}`,
  );
});
