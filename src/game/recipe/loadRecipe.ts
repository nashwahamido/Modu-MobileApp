...
export function composeRecipe(recipe: RecipeV1, assets: RecipeAssets, row: RecipeRow): RecipeResult {
  try {
    ...
    const furniture: Furniture = {
      meta: { id: asFurnitureId(recipe.id), thumbnail: assets.thumbnail, ...(recipe.mode ? { mode: recipe.mode } : {}), ...counts },
      model: assets.model,
      ...
      // sweep rides the recipe (pure data) so a cloud build keeps order-aware travel; `tools` is asset-bearing (icon/model requires) and is attached by the APP-side loader (cloudFurniture) — this module stays portal-packageable per purity.test
      ...(recipe.sweep ? { sweep: recipe.sweep as Furniture["sweep"] } : {}),
      components,
