// The Shop's purchase dialogs, per the mocks. PurchaseNoticeDialog is the "you can't buy this yet" notice: the item's price pill overlapping the preview well, the preview (a level star when level-locked), the reason line, and a Close button.
...
// anything priced (a bundle, a reward) and src/shop keeps no dependency on the data layer.
...
export function PurchaseNoticeDialog({ name, price, minLevel, block, onClose }: PurchaseNoticeDialogProps) {
  const styles = useStyles(makeStyles);
  return (
    <OverlaySheet size="dialog" onClose={onClose}>
      <View style={styles.previewWrap}>
        <View style={styles.preview}>
          {block === "level" ? <LevelBadge level={minLevel} size={72} /> : null}
        </View>
...
export function PurchaseConfirmDialog({ name, price, onConfirm, onClose }: PurchaseConfirmDialogProps) {
  const styles = useStyles(makeStyles);
  const t = useTheme();
  return (
    <OverlaySheet size="dialog" onClose={onClose}>
