// One source of truth for how the app clears Android's edge-to-edge system UI. Edge-to-edge is on (app.json edgeToEdgeEnabled), so every screen draws under the cutout and gesture bar and must pad itself back in. In LANDSCAPE — the app's only orientation — the punch-hole and the home-indicator sit on the LEFT and RIGHT, not the top, so left/right insets carry the weight; top/bottom still matter for the status area and the gesture pill.
//
